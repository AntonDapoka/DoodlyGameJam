...
- **Graffiti:** `GraffitiInitializerScript` seeds initial player and opponent spots. Opponent initial spots are selected only from inactive graffiti using a convex-hull (Jarvis march) perimeter check; if no subset satisfies the perimeter constraints, `GraffitiManagementInteractorScript` falls back to a plain random selection so the opponent always starts with at least one active spot. `GraffitiInitializerScript` uses `[FormerlySerializedAs]` to migrate old field names (`_graffitiAmountInitial`, `_graffitiPerimeterMin`, `_graffitiPerimeterMax`) to the current fields (`_opponentInitialAmount`, `_opponentPerimeterMin`, `_opponentPerimeterMax`). `GraffitiManagementInteractorScript` tracks valid/active spots. `GraffitiScript` acts as the model: it owns a trigger `CapsuleCollider`, fills `completionCurrent` while an object with `PlayerMarker` is inside and has line-of-sight to `graffitiCenter`, and raises events (`OnInteractionStarted`, `OnInteractionEnded`, `OnInteractionReset`, `OnProgressChanged`, `OnCompleted`, `OnStateChanged`). When the player leaves the trigger, `completionCurrent` drains gradually; the UI stays visible until it reaches 0, then `OnInteractionReset` fires and the UI hides.
...
- **Opponent:** `OpponentInteractorScript` picks target graffiti spots and moves toward them to convert them back to opponent graffiti. Targets are projected onto the NavMesh (`NavMesh.SamplePosition`) so the opponent can paint graffiti on walls and elevations from the nearest walkable point. If the distance-based finder returns no target, a fallback timer (`TryFindTargetFallback`) runs every second, picks any valid non-opponent graffiti spot, and temporarily enables wall-ignoring direct movement when the path is blocked or the navigator is stuck.
...
7. **Inappropriate debug output:** `CompassUIScript` still contains informal/offensive `Debug.Log` messages that should be removed before release. `GraffitiJarvisAlgorithmFinderScript` debug output has been replaced with `Debug.LogWarning` messages.
